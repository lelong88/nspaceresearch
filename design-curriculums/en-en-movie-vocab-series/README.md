# en-en Movie Vocab Series

## Collection: Learn Vocabulary Through Cinema

- **Collection ID**: `f5j2mwmv`
- **Language**: en-en (English speakers learning English)

## Curriculums

| # | Movie | Curriculum ID | vi-en Source ID |
|---|---|---|---|
| 0 | Forrest Gump (1994) | `xCXbmpHgPnzvcJMd` | `5MsWSZwcWGYpfnrO` |
| 1 | The Shawshank Redemption (1994) | `P1R8JcxDcKQfHJ9w` | `yCj2EZKIPTkFNqtS` |
| 2 | Dead Poets Society (1989) | `Hp6xZs2BHKHO1xr7` | `LLy5qjuLk0VZ7SIi` |
| 3 | The Pursuit of Happyness (2006) | `uOqL55TpLttY4lAa` | `XjJRTMHxnBXFiA31` |

## How Content Was Created

Each curriculum was created by a mirror script (`create_movie_N_<movie>.py`) that:
...
4. Validated structural properties (18 unique words, correct activity sequences, no strip keys, youtubeUrl present, contentTypeTags=["movie"]) before upload
5. Called `curriculum/create` API with `language="en"`, `userLanguage="en"`
6. All curriculums created as private (`is_public = false`)

The same 4 movies and youtubeUrls are shared with the vi-en movie series. Each en-en curriculum mirrors its vi-en counterpart with English UI text instead of Vietnamese.

An orchestrator script (`create_en_en_movie_collection.py`) created the collection and added all 4 curriculums directly to it (no series intermediary), then set display orders 0–3.
...
WHERE cci.curriculum_collection_id = 'f5j2mwmv'
ORDER BY c.display_order;

-- Get full content for any curriculum
SELECT content FROM curriculum WHERE id = 'xCXbmpHgPnzvcJMd';
```
...
- `contentTypeTags=["movie"]` and `youtubeUrl` at top level of content JSON
- `is_public = false`
