"""
validate_content.py — Children's curriculum content validator for vi-de curriculums.
...
Formats:
  - "beginner_mini": 1 session, 3-5 vocab words, no vocabLevel1/vocabLevel2/writingParagraph/vocabLevel3
  - "beginner_short": 4 sessions, 8-10 vocab words, no writingParagraph/vocabLevel3
  - "preintermediate_short": 4 sessions, 10-12 vocab words, no writingParagraph/vocabLevel3
...
# Additional activities forbidden in beginner_mini
...
def _validate_top_level(content):
    ...
    if not isinstance(tags, list):
        raise ValueError("'contentTypeTags' must be an array")
    if len(tags) != 0:
        raise ValueError(
            "'contentTypeTags' must be an empty array [] for children's curriculums"
        )
...
def _validate_vocab_list(data, loc, activity_type):
    ...
    for j, word in enumerate(vocab_list):
        ...
        # Check lowercase — Python str.islower() naturally supports
        # German umlauts (ä, ö, ü) and ß. Multi-word entries with
        # spaces/hyphens are valid (e.g., "wie viel", "nach hause gehen").
        # We check that alphabetic characters are all lowercase.
        if not _is_valid_lowercase(word):
            raise ValueError(
                f"{loc} ({activity_type}): vocabList[{j}] '{word}' must be lowercase"
            )


def _is_valid_lowercase(word):
    """
    Check that a vocab word is valid lowercase.
    - Must not be empty
    - All alphabetic characters must be lowercase
    - Spaces and hyphens are allowed (multi-word entries)
    - German umlauts (ä, ö, ü) and ß are naturally lowercase per Python str.islower()
    """
    if not word or not word.strip():
        return False
    # If the word has any alphabetic characters, they must all be lowercase
