# Design Document: Vietnamese-Japanese Beginner Curriculums

## Overview

This design covers the creation of 20 Japanese-learning curriculums for Vietnamese-speaking adults at the beginner level, organized into 1 collection and 3 series. The system consists of:

- **20 standalone Python scripts** — one per curriculum, each containing hand-crafted beginner Japanese content
...
The language pair is `userLanguage="vi"` (Vietnamese speakers), `language="ja"` (learning Japanese). All marketing text (titles, descriptions, previews) is in Vietnamese. All learner-facing content is bilingual: Vietnamese explanations with Japanese vocabulary in hiragana/katakana and romaji pronunciation guidance.
...
2. **Beginner-specific validator** — a new `validate_content.py` in `vi-ja-beginner-curriculums/` supporting `beginner_mini` and `beginner_short` formats. Key difference from the children's validator: **no lowercase enforcement for vocabList** since Japanese uses hiragana/katakana which don't have letter case. Different vocab count ranges (3-5 for mini, 8-10 for short). Same strip-key and structural checks.

3. **No tone_assigner module** — with 20 curriculums across 3 series, tone assignments can be hard-coded in each script and documented in the orchestrator. Manual assignment with variety checks is simpler and more transparent than a programmatic assigner.
...
5. **Scripts directory**: `vi-ja-beginner-curriculums/`

6. **Reading passage length in characters** — Japanese text is measured in characters rather than words: 40-60 characters for mini passages, 60-80 per session for short, 100-140 combined for the final session reading.

## Architecture

```mermaid
graph TD
    subgraph "vi-ja-beginner-curriculums/"
...
sequenceDiagram
    ...
    Orch->>API: create_collection("Tiếng Nhật Cho Người Mới Bắt Đầu", ...)
...
| 2 | Đếm Số Tiếng Nhật | Những Bước Đầu Tiên | vivid_scenario | quiet_awe |
...
| 9 | Ẩm Thực Nhật Bản | Khám Phá Cuộc Sống | bold_declaration | introspective_guide |
| 10 | Mua Sắm Ở Nhật | Khám Phá Cuộc Sống | surprising_fact | team_building_energy |
| 11 | Di Chuyển Ở Nhật | Khám Phá Cuộc Sống | vivid_scenario | warm_accountability |
| 12 | Khách Sạn Và Lưu Trú | Khám Phá Cuộc Sống | provocative_question | quiet_awe |
| 13 | Văn Phòng Nhật Bản | Khám Phá Cuộc Sống | metaphor_led | practical_momentum |
| 14 | Bốn Mùa Nhật Bản | Khám Phá Cuộc Sống | empathetic_observation | introspective_guide |
...
| 17 | Nhà Ở Nhật Bản | Mở Rộng Thế Giới | vivid_scenario | quiet_awe |
| 18 | Lễ Hội Nhật Bản | Mở Rộng Thế Giới | provocative_question | practical_momentum |
...
def validate(content: dict, format: str) -> None:
    """
    Validates curriculum content JSON for vi-ja beginner curriculums.
...
6. `vocabList` is array of strings, field name is `vocabList` (not `words`) — **NO lowercase enforcement** (Japanese hiragana/katakana don't have case)
...
sys.path.insert(0, "/home/ubuntu/nspaceresearch/design-curriculums/vi-ja-beginner-curriculums")
...
- Japanese vocabulary in hiragana/katakana with romaji pronunciation in introAudio scripts
...
```
Session 1:
  1. introAudio — welcome + teach all words with romaji, Vietnamese meaning, cultural context (200-350 words)
  2. viewFlashcards — all words
  3. speakFlashcards — all words
  4. reading — short Japanese passage in hiragana/katakana (40-60 characters)
  5. speakReading
  6. readAlong
...
Session 1 (Group 1, "Phần 1"):
  1. introAudio — welcome + teach group 1 words with romaji and Vietnamese meaning
      ...
      "activities": [
        {
          ...
          "description": "Giới thiệu 5 câu chào hỏi cơ bản trong tiếng Nhật",
          "data": {
            "text": "Xin chào bạn! Hôm nay chúng ta sẽ học 5 câu chào hỏi cơ bản nhất trong tiếng Nhật..."
          }
        },
        {
          "activityType": "viewFlashcards",
          "title": "Flashcards: Chào hỏi",
          "description": "Học 5 từ: こんにちは, ありがとう, すみません, おはよう, さようなら",
          "data": {
        ...
        },
        {
          ...
          "description": "おはよう。こんにちは。ありがとう。すみません。さようなら。",
          "data": {
            "text": "おはよう。きょうは いい てんき です。こんにちは。ありがとう ございます。すみません、えき は どこ ですか。さようなら、また あした。",
            "vocabList": ["こんにちは", "ありがとう", "すみません", "おはよう", "さようなら"]
          }
        },
        {
        ...
        },
        {
          ...
          "data": {
            "text": "Tuyệt vời! Bạn vừa học xong 5 câu chào hỏi cơ bản nhất trong tiếng Nhật..."
          }
        }
      ]
    }
...
```json
{
  ...
  "title": "Viết: Ẩm thực",
  "description": "Viết câu tiếng Nhật về ẩm thực",
  "data": {
    "vocabList": ["ごはん", "さかな", "にく", "やさい", "たまご"],
    "items": [
      {
        "prompt": "Viết một câu tiếng Nhật dùng từ 'ごはん' (gohan - cơm). Ví dụ: ごはん が すき です。(Gohan ga suki desu - Tôi thích cơm.) Hãy thay 'すき' bằng 'おいしい' (oishii - ngon) nhé!",
        "targetVocab": "ごはん"
      },
      {
        "prompt": "Viết một câu tiếng Nhật dùng từ 'さかな' (sakana - cá). Ví dụ: さかな を たべます。(Sakana wo tabemasu - Tôi ăn cá.) Hãy thay 'さかな' bằng 'にく' (niku - thịt) nhé!",
        "targetVocab": "さかな"
      }
    ]
  }
...
**Key difference from children's validator properties:** Property 6 (vocabList format) does NOT enforce lowercase because Japanese hiragana/katakana characters have no case distinction. The field must still be an array of strings using the name `vocabList` (not `words`), and must not be empty.
...
*For any* vocab activity (viewFlashcards, speakFlashcards, vocabLevel1, vocabLevel2), if `data.vocabList` is not an array, is empty, contains non-string elements, or uses the field name `words` instead of `vocabList`, `validate()` SHALL raise a ValueError. Note: lowercase is NOT enforced for Japanese vocabulary.
...
**Tag format:** Each test is tagged with a comment: `# Feature: vi-ja-beginner-curriculums, Property N: <property_text>`

The 8 correctness properties above are implemented as Hypothesis property tests in a `test_validate.py` file within `vi-ja-beginner-curriculums/`. Each property test generates random curriculum content structures using Hypothesis strategies and verifies the validator's behavior.

**Generator strategies needed:**
- `valid_curriculum(format)` — generates a structurally valid curriculum content dict for the given format, with Japanese hiragana/katakana strings in vocabList
- `random_activity(activity_type)` — generates a valid activity of the given type
- `random_vocab_list(n)` — generates a list of n random Japanese strings (hiragana/katakana characters)
...
**Key generator difference from children's tests:** The `random_vocab_list` strategy generates strings from the hiragana/katakana Unicode ranges (U+3040-U+309F for hiragana, U+30A0-U+30FF for katakana) rather than lowercase ASCII strings.
...
- Verify writingSentence items have Vietnamese prompt text with romaji and targetVocab (Req 3.3)
- Verify reading passages use hiragana/katakana only, no kanji (Req 2.4)
...
```sql
-- Count all 20 vi-ja beginner curriculums
SELECT COUNT(*) FROM curriculum
WHERE id IN (<list of 20 IDs>);

-- Verify language pair (all should be language=ja, user_language=vi)
...
WHERE cc.title = 'Tiếng Nhật Cho Người Mới Bắt Đầu';
```

### Smoke Tests

- Verify each script file exists in `vi-ja-beginner-curriculums/`
