# Design Document: Vietnamese-French Children's Curriculum

## Overview

This design covers the creation of 22 French-learning curriculums for Vietnamese children aged 6-10, organized into 1 NEW collection and 3 series. The system consists of:
...
The language pair is `userLanguage="vi"` (Vietnamese speakers), `language="fr"` (learning French). All marketing text (titles, descriptions, previews) is in Vietnamese, targeting parents. All learner-facing content uses a warm, playful, encouraging tone appropriate for children aged 6-10, with French words introduced with pronunciation guidance in Vietnamese and Vietnamese meaning.
...
2. **Children-specific validator** — a new `vi-fr-children-curriculum/validate_content.py` is needed because children's curriculums have different session counts (1 or 4), different vocab counts (3-5, 8-10, or 10-12), and explicitly forbid `writingParagraph` and `vocabLevel3`. The validator supports three formats via a `format` parameter.

3. **Orchestrator needed** — this is the first vi-fr children's batch, so a new collection "Tiếng Pháp Cho Bé 6-10 Tuổi" and 3 series must be created before any curriculums can be added.

4. **French-specific adaptations** — vocabList uses standard French lowercase words with accents preserved (e.g., "éléphant", "forêt", "garçon"). Multi-word vocab items are allowed (e.g., "au revoir", "petit-déjeuner", "se réveiller"). Reading passages are entirely in French. introAudio scripts are primarily in Vietnamese with French pronunciation guidance using simple Vietnamese phonetic approximations.
...
```mermaid
graph TD
    subgraph "vi-fr-children-curriculum/"
...
sequenceDiagram
    ...
    Orch->>API: create_collection("Tiếng Pháp Cho Bé 6-10 Tuổi", ...)
    ...
    loop For each curriculum (1-22)
        ...
        Script->>API: create_curriculum(content, "fr", "vi")
...
**French-specific note:** The lowercase check for vocabList naturally supports French accented characters (é, è, ê, à, â, ç, ô, î, ù, û, ë, ï, œ, æ) since Python's `str.islower()` returns True for accented lowercase letters. Multi-word entries (e.g., "au revoir", "petit-déjeuner") are valid — the check is per-string, not per-word.
...
sys.path.insert(0, "/home/ubuntu/nspaceresearch/design-curriculums/vi-fr-children-curriculum")
...
def main():
    ...
    curriculum_id = create_curriculum(content, "fr", "vi")
...
| 8 | Món Ăn Pháp | Xây Nền Vững Chắc | 2 | beginner_short | vivid_scenario | team_building_energy |
...
| 22 | Đại Dương Xanh | Khám Phá Thêm | 8 | preintermediate_short | metaphor_led | warm_accountability |
...
```
Session 1:
  1. introAudio — welcome + teach all words with French pronunciation guidance and Vietnamese meaning (200-350 words)
  2. viewFlashcards — all words
  3. speakFlashcards — all words
  4. reading — short French passage (40-60 words)
  5. speakReading
  6. readAlong
...
```
Session 1 (Group 1):
  1. introAudio — welcome + teach group 1 words (French word + pronunciation + Vietnamese meaning)
  2. viewFlashcards (group 1)
  3. speakFlashcards (group 1)
  4. vocabLevel1 (group 1)
  5. reading — French passage using group 1 words (60-80 words)
  6. readAlong
...
Session 2 (Group 2):
  ...
  5. reading — French passage using group 2 words (60-80 words)
  6. readAlong
  7. introAudio — session wrap-up
...
Session 4 (Final):
  ...
  2. reading — combined French passage (100-120 words)
  3. speakReading
  4. readAlong
...
Session 1 (Group 1):
  ...
  6. reading — French passage using group 1 words (80-100 words)
  7. speakReading
  8. readAlong
...
Session 2 (Group 2):
  ...
  6. reading — French passage using group 2 words (80-100 words)
  7. speakReading
  8. readAlong
  9. introAudio — session wrap-up
...
Session 4 (Final):
  1. introAudio — final reading intro
  2. reading — combined French passage (150-180 words)
  3. speakReading
  4. readAlong
      ...
      "activities": [
        {
          ...
          "description": "Giới thiệu bài học về màu sắc bằng tiếng Pháp",
          "data": {
            "text": "Xin chào các bé! Hôm nay chúng ta sẽ học về màu sắc bằng tiếng Pháp nè! Từ đầu tiên là rouge — đọc là 'ru-giơ' nhé — nghĩa là màu đỏ. Ví dụ: Le ballon est rouge — quả bóng màu đỏ..."
          }
        },
        {
          "activityType": "viewFlashcards",
          "title": "Flashcards: Màu sắc",
          "description": "Học 5 từ: rouge, bleu, vert, jaune, blanc",
          "data": {
        ...
        },
        {
          ...
          "description": "Đọc đoạn văn tiếng Pháp về màu sắc",
          "data": {
            "text": "Le ciel est bleu. Le soleil est jaune. L'herbe est verte. La fleur est rouge. Le nuage est blanc.",
            "vocabList": ["rouge", "bleu", "vert", "jaune", "blanc"]
          }
        },
        {
        ...
        },
        {
          ...
          "data": {
            "text": "Các bé ơi, hôm nay chúng ta đã học được 5 màu sắc bằng tiếng Pháp rồi nè! Con giỏi lắm!..."
          }
        }
      ]
    }
...
```json
{
  ...
  "description": "Viết câu tiếng Pháp về trường học",
  "data": {
    "vocabList": ["école", "livre", "crayon", "table", "maître"],
    "items": [
      {
        "prompt": "Viết một câu tiếng Pháp dùng từ 'école'. Ví dụ: Je vais à l'école. (Tôi đi đến trường.) Bé hãy thay 'je' bằng một người khác nhé! Ví dụ: Marie va à l'école.",
        "targetVocab": "école"
      },
      {
        "prompt": "Viết một câu tiếng Pháp dùng từ 'livre'. Ví dụ: Le livre est rouge. (Quyển sách màu đỏ.) Bé hãy thay 'rouge' bằng một màu khác nhé!",
        "targetVocab": "livre"
      }
    ]
  }
...
|---|---|
| `curriculum/create` | `firebaseIdToken`, `language: "fr"`, `userLanguage: "vi"`, `content: JSON.stringify(content)` |
...
|---|---|---|---|
| 1 | Thế Giới Màu Sắc | rouge, bleu, vert, jaune, blanc | 5 |
| 2 | Đếm Từ 1 Đến 5 | un, deux, trois, quatre, cinq | 5 |
| 3 | Gia Đình Yêu Thương | maman, papa, frère, soeur, bébé | 5 |
| 4 | Trái Cây Ngon Lành | pomme, banane, orange, fraise, raisin | 5 |
| 5 | Bạn Thú Cưng | chien, chat, poisson, oiseau, lapin | 5 |
| 6 | Chào Hỏi Vui Vẻ | bonjour, merci, au revoir, pardon, oui | 5 |
| 7 | Một Ngày Ở Trường | école, livre, crayon, table, maître, classe, cahier, cartable, récréation, devoirs | 10 |
| 8 | Món Ăn Pháp | pain, fromage, croissant, soupe, lait, poulet, gâteau, salade, eau, chocolat | 10 |
| 9 | Sân Chơi Vui Nhộn | courir, sauter, jouer, rire, chanter, dessiner, ami, ballon, toboggan, balançoire | 10 |
| 10 | Cơ Thể Của Em | tête, main, pied, yeux, oreille, bouche, nez, ventre, cheveux, dent | 10 |
| 11 | Thời Tiết Hôm Nay | soleil, pluie, vent, froid, chaud, neige, nuage, orage, arc-en-ciel, parapluie | 10 |
| 12 | Tủ Quần Áo | chemise, pantalon, robe, chaussures, chapeau, chaussettes, manteau, jupe, écharpe, gants | 10 |
| 13 | Xe Cộ Quanh Em | voiture, bus, train, avion, vélo, bateau, métro, moto, rapide, lent | 10 |
| 14 | Con Vật Đáng Yêu | éléphant, lion, girafe, singe, tigre, ours, pingouin, serpent, zèbre, perroquet | 10 |
| 15 | Khám Phá Thiên Nhiên | forêt, rivière, montagne, papillon, fleur, feuille, arbre, ciel, étoile, lune, jardin, herbe | 12 |
| 16 | Bữa Ăn Gia Đình | petit-déjeuner, déjeuner, dîner, assiette, fourchette, couteau, cuillère, verre, cuisine, délicieux, manger, boire | 12 |
| 17 | Bốn Mùa Trong Năm | printemps, été, automne, hiver, nager, ski, plage, feuilles, fleurir, bonhomme de neige, vacances, chaleur | 12 |
| 18 | Đi Chợ Cùng Mẹ | acheter, vendre, argent, combien, cher, marché, légumes, fruits, sac, payer, magasin, prix | 12 |
| 19 | Sinh Hoạt Hàng Ngày | se réveiller, se brosser les dents, se laver, matin, dormir, après-midi, rentrer, lire, goûter, soir, nuit, bonne nuit | 12 |
| 20 | Ngôi Nhà Của Em | maison, chambre, salon, salle à manger, salle de bain, porte, fenêtre, lit, escalier, toit, clé, mur | 12 |
| 21 | Thể Thao Sôi Động | football, natation, basket, tennis, danse, grimper, lancer, attraper, but, gagner, équipe, champion | 12 |
| 22 | Đại Dương Xanh | baleine, dauphin, requin, pieuvre, tortue, crabe, méduse, coquillage, corail, vague, plongée, sable | 12 |

**Overlap verification:** Zero overlap between all 22 vocabList arrays (each curriculum uses unique French entries).
...
**Tag format:** Each test is tagged with a comment: `# Feature: vi-fr-children-curriculum, Property N: <property_text>`
...
- `random_vocab_list(n)` — generates a list of n random lowercase French-style strings (including accented characters)
...
**French-specific generator considerations:**
- `random_vocab_list` must generate strings that include French accented characters (é, è, ê, à, â, ç, ô, î, ù, û) to verify the validator correctly accepts them as lowercase
- Multi-word entries with spaces and hyphens (e.g., "au revoir", "petit-déjeuner") should be generated to verify they pass validation
...
- Verify French accented words pass the lowercase validation (Req 14.5, 14.6)
...
```sql
-- Count all vi-fr children's curriculums (expect 22)
...
- Verify each script file exists in `vi-fr-children-curriculum/`
- Verify no script calls `setPublic` (Req 12.1)
- Verify orchestrator creates exactly 1 collection and 3 series
- Verify validator file exists at `vi-fr-children-curriculum/validate_content.py`
