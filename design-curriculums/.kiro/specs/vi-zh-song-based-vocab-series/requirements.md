...
Create a new collection and song-based vocabulary series for learning Chinese (Mandarin) vocabulary through popular Chinese song lyrics. Instead of topic-based reading passages, each curriculum uses verbatim song lyrics as the reading content. Vocabulary words are drawn from the lyrics of carefully selected evergreen Chinese songs that are clear, comprehensible, and appropriate for pre-intermediate to intermediate Vietnamese learners of Chinese.

A brand new collection — "Học Từ Vựng Tiếng Trung Qua Âm Nhạc (通过音乐学中文词汇)" — is created via `curriculum-collection/create` to house the song series. This collection is separate from the existing vi-en music collection because it targets a different language pair (vi-zh).

Each curriculum is built around a single Chinese song. The content JSON includes a `youtubeUrl` field linking to the song on YouTube. Songs must be evergreen (not trending/ephemeral), have clear Mandarin pronunciation (no heavy dialect, no rap), and contain lyrics at the HSK 3-4 level for pre-intermediate to intermediate vi-zh learners. The series follows the same 18-word / 5-session structure as the existing vi-en song series, with the key difference that reading passages are Chinese song lyrics and all user-facing text is in Vietnamese with Chinese vocabulary.
...
**Series:** "Học Từ Vựng Qua Bài Hát Trung Quốc (通过中文歌曲学词汇)" — display_order 100 within the new collection. Each curriculum is one song, with 4 curriculums in the initial series.
...
- **Persuasive_Copy**: The required emotional sales copy style for all learner-facing text (descriptions, previews, introAudio scripts), written in Vietnamese.
- **API**: The helloapi.step.is REST API used for all CRUD operations on curriculums, series, and collections.
- **Evergreen_Song**: A song that has enduring popularity and cultural relevance, not a short-lived trend. Examples: classic Mandopop ballads, folk-pop, and iconic songs from established Chinese/Taiwanese artists.
- **HSK**: Hanyu Shuiping Kaoshi — the standardized Chinese proficiency test. HSK 3-4 corresponds roughly to A2-B1 level.

## Requirements

### Requirement 1: Collection and Series Creation

**User Story:** As a content manager, I want to create a new collection and a Chinese song-based vocabulary series within it, so that Vietnamese learners of Chinese can discover and access song-based vocabulary learning as a distinct content category on the platform.
...
2. WHEN the series is created, THE API SHALL be called via `curriculum-series/create` with the title "Học Từ Vựng Qua Bài Hát Trung Quốc (通过中文歌曲学词汇)", a persuasive Vietnamese description (under 255 characters), and `isPublic: true`.
...
**User Story:** As a content manager, I want Chinese songs to be carefully selected for language-learning suitability, so that the lyrics serve as effective reading material for pre-intermediate to intermediate Vietnamese learners of Chinese.

#### Acceptance Criteria

1. THE Curriculum SHALL be based on a Chinese song that is evergreen — having enduring popularity and cultural relevance beyond a single chart cycle.
2. THE Curriculum SHALL be based on a song with clear Mandarin pronunciation and enunciation by the vocalist, excluding rap, heavy dialect singing, or genres where lyrics are difficult to follow audibly.
3. THE Curriculum SHALL be based on a song whose lyrics are predominantly at the HSK 3-4 level — containing vocabulary appropriate for pre-intermediate to intermediate learners.
...
**User Story:** As a content manager, I want each curriculum's reading passages to be the verbatim Chinese lyrics of the selected song, so that learners read and study real song text rather than authored articles.

#### Acceptance Criteria

1. THE Curriculum reading activities SHALL use verbatim song lyrics in simplified Chinese as the reading text, not paraphrased or summarized versions.
...
**User Story:** As a content manager, I want each song curriculum to follow the established 18-word / 5-session structure, so that the learning experience is consistent with the rest of the platform.

#### Acceptance Criteria

1. THE Curriculum SHALL contain exactly 18 vocabulary words (Chinese words/phrases) drawn from the song lyrics, divided into 3 groups of 6 words (W1, W2, W3) across 3 learning sessions.
...
**User Story:** As a content manager, I want vocabulary words to be drawn directly from the Chinese song lyrics, so that learners encounter these words naturally when reading and listening to the song.

#### Acceptance Criteria

1. THE Curriculum SHALL select 18 Chinese vocabulary words/phrases that appear in the song lyrics and are useful for HSK 3-4 level Vietnamese learners of Chinese.
2. THE Curriculum SHALL prioritize words that are: (a) genuinely present in the lyrics, (b) at the HSK 3-4 level, (c) broadly useful beyond the song context, and (d) teachable with clear definitions and example sentences.
3. THE Curriculum SHALL NOT select single-character function words (的, 了, 在, 是) or extremely common words as vocabulary items unless they appear in an idiomatic or phrasal context worth teaching.
...
1. THE Curriculum description SHALL follow the 5-beat persuasive copy structure, adapted to reference the Chinese song, artist, and the emotional connection between music and language learning.
...
3. WHEN an introAudio activity is created for Sessions 1–3, THE introAudio script SHALL reference the song title, artist, and the context of the lyrics when teaching vocabulary — explaining how each Chinese word appears in the song and what it means in that lyrical context. Definitions and explanations are in Vietnamese.
...
6. THE Curriculum SHALL use Vietnamese for all user-facing text (title, description, preview, introAudio scripts, writing prompts) and simplified Chinese for the song lyrics in reading activities.

### Requirement 7: Content Quality — Writing Prompts (Song-Themed)

**User Story:** As a content manager, I want writing prompts to connect vocabulary to the song's themes, so that learners practice Chinese words in contexts inspired by the music they just studied.

#### Acceptance Criteria

1. THE writingSentence activity SHALL provide a detailed prompt in Vietnamese specifying a context related to the song's themes for using the Chinese word, plus an example sentence in Chinese, in the format: "Sử dụng từ 'X' để nói về [specific context related to the song's themes]. Ví dụ: [full example sentence in Chinese]."
...
2. WHEN a viewFlashcards, speakFlashcards, vocabLevel1, vocabLevel2, or vocabLevel3 activity is created, THE Activity title SHALL follow the format "Flashcards: <song-related topic>" and the description SHALL list the Chinese words being learned.
...
**User Story:** As a content manager, I want every piece of learner-facing text to be individually crafted per song, so that each curriculum reads as if written by someone who deeply understands that specific Chinese song and its lyrics.
...
3. THE Series title and description SHALL be written in Vietnamese with Chinese elements, consistent with the bilingual convention for vi-zh HSK 3-4 content.
...
1. THE Curriculum title SHALL include both the song title (in Chinese) and artist name, formatted bilingually (Vietnamese framing + Chinese song title + artist name), e.g., "Học Qua Bài Hát: '月亮代表我的心' – 邓丽君".
...
**User Story:** As a content manager, I want the vi-zh song series to have its own folder with a README tracking creation details, so that content is recoverable and auditable.

#### Acceptance Criteria

1. THE workspace SHALL contain a dedicated folder `vi-zh-song-based-vocab-series/` for the series.
