# Design Document: Movie-Based Vocab Series

## Overview

This feature creates a brand new collection "Học Từ Vựng Qua Điện Ảnh (Learn Vocabulary Through Cinema)" and populates it with a single series "Học Từ Vựng Qua Phim (Learn Vocabulary Through Movies)" containing 4 curriculums. Each curriculum is built around a different iconic movie scene, using verbatim movie dialogue or monologue as reading passages instead of authored articles or song lyrics. Movie scenes are selected for A2-B1 level dialogue appropriate for pre-intermediate to intermediate Vietnamese learners of English.
...
1. **New collection via API** — The orchestrator calls `curriculum-collection/create` to create the cinema collection, then `curriculum-series/create` for the series, then wires them together. Same pattern as the song-based vocab series.
2. **One script per curriculum** — Same pattern as song-based and health-wellness series. Each script is ~500-800 lines with all hand-written content.
3. **One orchestrator for collection + series** — A single `create_movie_series.py` handles collection creation, series creation, adding curriculums, setting display orders, and wiring the series into the collection.
4. **Movie dialogue as reading passages** — Reading activities use verbatim dialogue/monologue sourced via web search. Sessions 1-3 use dialogue portions containing that session's vocabulary; Sessions 4-5 use the complete dialogue.
...
7. **Session structure matches song-based series exactly** — Sessions 1-3 have 12 activities (introAudio ×2, viewFlashcards, speakFlashcards, vocabLevel1-3, introAudio, reading, speakReading, readAlong, writingSentence). Session 4 has 4 activities. Session 5 has 5 activities.

## Architecture

```mermaid
graph TD
    subgraph NewCollection["Học Từ Vựng Qua Điện Ảnh (NEW collection)"]
        S1["Học Từ Vựng Qua Phim — order 100"]
    end

    S1 --> C1["Movie 1 (order 0)"]
    S1 --> C2["Movie 2 (order 1)"]
    S1 --> C3["Movie 3 (order 2)"]
    S1 --> C4["Movie 4 (order 3)"]
```
...
sequenceDiagram
    participant User
    participant CurrScript as create_movie_N_<movie>.py
    participant Orch as create_movie_series.py
    participant API as helloapi.step.is

    Note over User,API: Phase 1: Create curriculums (run each script individually)
    User->>CurrScript: python create_movie_1_<movie>.py
    ...
    User->>Orch: python create_movie_series.py
...
├── create_movie_1_<movie>.py       # Curriculum script for movie 1
├── create_movie_2_<movie>.py       # Curriculum script for movie 2
├── create_movie_3_<movie>.py       # Curriculum script for movie 3
├── create_movie_4_<movie>.py       # Curriculum script for movie 4
└── create_movie_series.py          # Orchestrator (collection + series + wiring)
```
...
Each `create_movie_N_<movie>.py` script:
...
4. Defines reading passages from verbatim movie dialogue: `DIALOGUE_1` (portion for session 1), `DIALOGUE_2` (portion for session 2), `DIALOGUE_3` (portion for session 3), `FULL_DIALOGUE` (complete scene dialogue/monologue)
...
`create_movie_series.py`:

1. Takes 4 curriculum IDs as constants (pasted from curriculum script output)
2. Calls `POST curriculum-collection/create` with title "Học Từ Vựng Qua Điện Ảnh (Learn Vocabulary Through Cinema)", persuasive Vietnamese description, `isPublic: true`
3. Calls `POST curriculum-series/create` with title "Học Từ Vựng Qua Phim (Learn Vocabulary Through Movies)", Vietnamese description (≤255 chars), `isPublic: true`
...
| `curriculum-collection/create` | Create the new cinema collection | SuperAuthGuard |
| `curriculum-series/create` | Create the movie series | SuperAuthGuard |
...
### Curriculum Content Structure (Movie-Adapted)

```python
content = {
    "title": "Học Qua Phim: 'Movie Title' – Scene description",
    "description": "Multi-paragraph persuasive copy in Vietnamese (5-beat structure, movie-adapted)",
    "preview": {
        "text": "~150 word vivid marketing copy referencing the movie scene and its themes"
    },
    "youtubeUrl": "https://www.youtube.com/watch?v=XXXXXXXXXXX",
    "learningSessions": [
        # Session 1-3: Learning sessions (6 words each, dialogue as reading)
        {
            "title": "Buổi 1: <dialogue theme excerpt>",
            "activities": [
                # introAudio (welcome + movie scene context)
                # introAudio (vocab teaching — how each word appears in the dialogue)
                # viewFlashcards, speakFlashcards
                # vocabLevel1, vocabLevel2, vocabLevel3
                # introAudio (grammar/usage notes from dialogue)
                # reading (verbatim dialogue portion), speakReading, readAlong
                # writingSentence (movie-themed prompts)
            ]
        },
        # Session 4: Review (all 18 words)
        {
            "title": "Ôn tập",
            "activities": [
                # introAudio (congratulations + recap of movie scene themes)
        ...
        # Session 5: Full dialogue reading + farewell
        {
            ...
            "activities": [
                ...
                # reading (FULL dialogue), speakReading, readAlong
...
{
    "targetVocab": "word",
    "prompt": "Sử dụng từ 'word' để nói về [specific context related to the movie scene's themes]. Ví dụ: [full example sentence]."
}
```
...
### Key Differences from Song-Based Series

| Aspect | Song-Based | Movie-Based |
|---|---|---|
| Collection | Học Từ Vựng Qua Âm Nhạc | Học Từ Vựng Qua Điện Ảnh (new) |
| Series | Học Từ Vựng Qua Bài Hát | Học Từ Vựng Qua Phim |
| Reading passages | Verbatim song lyrics | Verbatim movie dialogue/monologue |
| Reading passage vars | LYRICS_1, LYRICS_2, LYRICS_3, FULL_LYRICS | DIALOGUE_1, DIALOGUE_2, DIALOGUE_3, FULL_DIALOGUE |
| Curriculum title format | "Học Qua Bài Hát: 'Song' – Artist" | "Học Qua Phim: 'Movie' – Scene description" |
| Script naming | create_song_N_<artist>.py | create_movie_N_<movie>.py |
| Orchestrator | create_song_series.py | create_movie_series.py |
...
| Session 1-3 reading | Lyric portions (verses/chorus) | Dialogue portions per session |
| Session 5 reading | Full song lyrics | Full scene dialogue/monologue |
| Folder | song-based-vocab-series/ | movie-based-vocab-series/ |
...
### Property 8: Vocabulary words appear in full dialogue

*For any* curriculum, every one of the 18 vocabulary words SHALL appear (case-insensitive) in the `FULL_DIALOGUE` text used for the complete scene reading.
...
### Property 10: Curriculum title contains movie title and no difficulty level

*For any* curriculum, the `title` field in the content dict SHALL contain the movie title as a substring, and SHALL NOT contain difficulty level descriptors (e.g., "Upper-Intermediate", "Advanced", "Beginner").
...
6. Verify all 18 vocab words appear in FULL_DIALOGUE (case-insensitive)
...
10. Verify curriculum title contains movie title
...
```sql
-- Find the new cinema collection
SELECT id, title, description, is_public
FROM curriculum_collections
WHERE title LIKE '%Điện Ảnh%';
...
- [ ] All 18 words appear in the full movie dialogue
...
- [ ] Curriculum title contains movie title and scene identifier
- [ ] Writing prompts contain target vocab and "Ví dụ:"
- [ ] Farewell introAudio contains all 18 vocabulary words
- [ ] Session 1-3 reading texts are substrings of full dialogue
- [ ] Session 5 reading text is the complete dialogue
