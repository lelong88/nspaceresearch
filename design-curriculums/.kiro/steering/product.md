...
- When using an existing curriculum as a reference/template, always strip these keys from the copied content before using it as input for a NEW curriculum
- When updating an existing curriculum, PRESERVE all generated keys. Only modify the specific fields you intend to change (e.g., rewrite introAudio text, fix a description). Do NOT fetch → strip → push back. If you need to re-trigger generation for specific activities, use `curriculum/removeGeneratedContent` with the appropriate `activityIndices`
...
**CRITICAL — Never strip keys on existing curriculums via `curriculum/update`:**
Stripping generated keys is ONLY for preparing content for `curriculum/create` (new curriculums) or when copying content to use as a template. NEVER fetch an existing curriculum's content, strip keys, and push it back via `curriculum/update`. This destroys all generated metadata (`taskId`, `illustrationSet`, `curriculumTags`, `mp3Url`, `segments`, `lessonUniqueId`, `imageId`, `userReadingId`, etc.) that was produced by the content generation pipeline — requiring a full re-generation of audio, illustrations, and other assets. When updating specific fields on an existing curriculum (e.g., rewriting `introAudio` text, fixing a `description`), only modify the targeted fields and preserve all other keys intact. Use `curriculum/removeGeneratedContent` if you intentionally need to trigger re-generation for specific activities.
...
Every activity must have a `practiceMinutes` field (integer). This tells the client how long the activity is expected to take. Default values by activity type:
