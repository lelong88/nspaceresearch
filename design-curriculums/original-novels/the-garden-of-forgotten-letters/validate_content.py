#!/usr/bin/env python3
"""
Validation script for The Garden of Forgotten Letters curriculum content.
Implements all 14 correctness properties.
Run from: original-novels/the-garden-of-forgotten-letters/
"""
...
def check_p1_session_structure(cur, ch, violations):
    ...
    for i in range(5):
        ...
        if len(acts) != 3:
            violations.append(f"FAIL [Chapter {ch}, Session {i+1}]: Expected 3 activities, found {len(acts)}")
            continue
        types = [a.get("activityType") for a in acts]
        if types != expected_15:
            violations.append(f"FAIL [Chapter {ch}, Session {i+1}]: Activity types {types} != {expected_15}")
    acts6 = sessions[5].get("activities", [])
    expected_6 = ["viewFlashcards", "readAlong"]
    if len(acts6) != 2:
        violations.append(f"FAIL [Chapter {ch}, Session 6]: Expected 2 activities, found {len(acts6)}")
    else:
        types6 = [a.get("activityType") for a in acts6]
        if types6 != expected_6:
            violations.append(f"FAIL [Chapter {ch}, Session 6]: Activity types {types6} != {expected_6}")
...
def check_p2_vocab_counts(cur, ch, violations):
    ...
    for i in range(5):
        ...
        if len(words) != 3:
            violations.append(f"FAIL [Chapter {ch}, Session {i+1}, viewFlashcards]: Expected 3 vocab words, found {len(words)}")
        all_words.extend(words)
    fc6 = sessions[5]["activities"][0]
    words6 = fc6.get("data", {}).get("vocabList", [])
    if len(words6) != 15:
        violations.append(f"FAIL [Chapter {ch}, Session 6, viewFlashcards]: Expected 15 vocab words, found {len(words6)}")
    unique = set(w.lower() for w in all_words)
    if len(unique) != 15:
        violations.append(f"FAIL [Chapter {ch}]: Expected 15 unique vocab words, found {len(unique)}")
    ...
    for i in range(5):
        ...
        for w in words:
            if w.lower() not in passage_lower:
                violations.append(f"FAIL [Chapter {ch}, Session {i+1}, reading]: Vocab word '{w}' not found in passage text")
    ...
    for ch, cur in all_chapters:
        ...
        for i in range(5):
            ...
            for w in words:
                wl = w.lower()
                if wl in seen:
                    violations.append(f"FAIL [Chapter {ch}]: Vocab word '{w}' also appears in Chapter {seen[wl]}")
                else:
                    seen[wl] = ch
    ...
    for i in range(5):
        ...
        for w in words:
            if not isinstance(w, str) or not w.strip():
                violations.append(f"FAIL [Chapter {ch}, Session {i+1}, viewFlashcards]: Vocab entry is empty or not a string: {w!r}")
    words6 = sessions[5]["activities"][0].get("data", {}).get("vocabList", [])
    for w in words6:
        if not isinstance(w, str) or not w.strip():
            violations.append(f"FAIL [Chapter {ch}, Session 6, viewFlashcards]: Vocab entry is empty or not a string: {w!r}")
...
def check_p6_readalong_matches_reading(cur, ch, violations):
    ...
    for i in range(5):
        ...
        if reading_text != readalong_text:
            violations.append(f"FAIL [Chapter {ch}, Session {i+1}, readAlong]: Text does not match reading activity text")
...
def check_p7_session6_full_text(cur, ch, violations):
    ...
    if actual != expected:
        violations.append(f"FAIL [Chapter {ch}, Session 6, readAlong]: Full chapter text does not match concatenation of 5 passages")
    ...
    for i in range(6):
        ...
        for act in acts:
            ...
            if atype in ("viewFlashcards", "reading"):
                ...
                if speed != -0.2:
                    violations.append(f"FAIL [Chapter {ch}, Session {i+1}, {atype}]: audioSpeed = {speed}, expected -0.2")
...
def check_p9_passage_word_count(cur, ch, violations):
    ...
    for i in range(5):
        ...
        wc = len(text.split())
        if wc < 135 or wc > 220:
            violations.append(f"FAIL [Chapter {ch}, Session {i+1}, reading]: Passage word count {wc} outside range 135-220")
...
def check_p10_title_format(cur, ch, violations):
    ...
    if pattern not in title:
        violations.append(f"FAIL [Chapter {ch}]: Title does not contain '{pattern}': {title!r}")
    if not VIETNAMESE_RE.search(title):
        violations.append(f"FAIL [Chapter {ch}]: Title does not contain Vietnamese characters")
    if "(" not in title or ")" not in title:
        violations.append(f"FAIL [Chapter {ch}]: Title missing parenthesized English title")
...
def check_p11_session_titles(cur, ch, violations):
    ...
    for i in range(5):
        ...
        if actual != expected:
            violations.append(f"FAIL [Chapter {ch}, Session {i+1}]: Title = {actual!r}, expected {expected!r}")
    actual6 = sessions[5].get("title", "")
    if actual6 != "Ôn tập":
        violations.append(f"FAIL [Chapter {ch}, Session 6]: Title = {actual6!r}, expected 'Ôn tập'")
...
def check_p12_activity_title_formats(cur, ch, violations):
    ...
    for i in range(6):
        ...
        for act in acts:
            ...
            if not title.strip():
                violations.append(f"FAIL [Chapter {ch}, Session {i+1}, {atype}]: Empty title")
            if not desc.strip():
                violations.append(f"FAIL [Chapter {ch}, Session {i+1}, {atype}]: Empty description")
            if atype == "viewFlashcards" and not title.startswith("Flashcards:"):
                violations.append(f"FAIL [Chapter {ch}, Session {i+1}, viewFlashcards]: Title does not start with 'Flashcards:': {title!r}")
            if atype == "reading" and not title.startswith("Đọc:"):
                violations.append(f"FAIL [Chapter {ch}, Session {i+1}, reading]: Title does not start with 'Đọc:': {title!r}")
            if atype == "readAlong" and i < 5 and not title.startswith("Nghe:"):
                violations.append(f"FAIL [Chapter {ch}, Session {i+1}, readAlong]: Title does not start with 'Nghe:': {title!r}")
            if atype == "readAlong" and i == 5 and "Toàn bộ câu chuyện" not in title:
                violations.append(f"FAIL [Chapter {ch}, Session 6, readAlong]: Title does not contain 'Toàn bộ câu chuyện': {title!r}")
...
def check_p13_vietnamese_metadata(cur, ch, violations):
    ...
    if not VIETNAMESE_RE.search(preview_text):
        violations.append(f"FAIL [Chapter {ch}]: preview.text does not contain Vietnamese characters")
    if not VIETNAMESE_RE.search(description):
        violations.append(f"FAIL [Chapter {ch}]: description does not contain Vietnamese characters")
    if cur.get("language") != "en":
        violations.append(f"FAIL [Chapter {ch}]: language = {cur.get('language')!r}, expected 'en'")
    if cur.get("userLanguage") != "vi":
        violations.append(f"FAIL [Chapter {ch}]: userLanguage = {cur.get('userLanguage')!r}, expected 'vi'")
        ...
        if isinstance(obj, dict):
            for key, val in obj.items():
                if key in STRIP_KEYS:
                    violations.append(f"FAIL [Chapter {ch}]: Forbidden key '{key}' found at {path}.{key}")
